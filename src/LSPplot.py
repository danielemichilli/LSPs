...
def sp_plot(pulses,rfi,meta_data,top_candidates,best_pulses,sap,beam,store,events=pd.DataFrame()):
  ...
  if not events.empty: ax1.scatter(events.Time, events.DM, facecolors='none', s=sig, c='k', linewidths=[0.5,])
  ...
  if len(pulses.DM.unique())>1:
    ax2.hist(pulses.DM.tolist(),bins=300,histtype='stepfilled',color=u'k')
    ax2.set_xscale('log')
    ax2.set_xlabel('DM (pc/cm3)')
    ax2.set_ylabel('Counts')
    ax2.set_xlim(5,550)
  
    ax4.hist(pulses.Sigma.tolist(),bins=100,histtype='step',color='k')
    ax4.set_xlabel('SNR')
  ...
  ax3.axis([5,550,pulses.Sigma.min(),pulses.Sigma.max()+3.])
